...
FileSystem.create = function(paths) {
  ...
  defaults.forEach(function(dir) {
      ...
      while (parts.length > 0) {
          currParts.push(parts.shift());
          var dirPath = currParts.join('.');
          var pathLoc = dotty.get(paths, dirPath);
          if ((typeof pathLoc === 'function' && (pathLoc.name === 'createFile' || pathLoc.name === 'createSymLink')) ||
              typeof pathLoc === 'string') {
              throw new Error('Conflict detected at ' + dirPath);
          } else if (pathLoc) {
              //skip if it exists
          } else {
              dotty.put(paths, dirPath, {});
          }
      }
  });
